...
1. **정신과 의사선생님** 또는 **심리상담사님**께 보여드리기 좋은 방식이에요.
2. **그동안의 분석 결과를 한번에 확인하기** 좋은 방식이에요.
3. **다른 방식 대비 내용 수정이 쉽지 않기 때문**이에요.
         
해당 심리분석 결과는 AI가 판단한 것이기 때문에 정확하지 않아요!
따라서 고객님에 대한 판단은 정신과 의사선생님 혹은 심리상담사님께서 좀더 정확히 아실거에요.
""")
col1,col2,col3=st.columns([1,8,1])
with col2:
    if st.button('고객님의 이메일로 결과를 한번 받아보시겠어요?'):
